 ...
 */
package bbct.android.common.test.lib;
...
final public class BBCTTestUtil {
    ...
    public static void sendKeysToCardDetails(Solo solo, BaseballCard card,
        ...
        String pathToPictureFront = card.getPathToPictureFront();
        if (fieldFlags.contains(EditTexts.CARD_FRONT_IMAGE) && pathToPictureFront.equals("") == false) {
            frontImage = (ImageView) solo.getView(R.id.image_card_details_front);
            Activity activity = solo.getCurrentActivity();
            frontPic = picHelper.GetScaledImageFromPath(pathToPictureFront, 120, 120);
            ...
            */
            /*solo.clickOnView(frontImage);// frontImage.setImageBitmap(frontPic);
            solo.waitForCondition(new Condition() {
                @Override
                public boolean isSatisfied() {
                    boolean focus = !activity.hasWindowFocus();
                    return focus;
                }
            }, 100000);
            solo.clickOnScreen(0, 10);*/
        //}
        
        /*String pathToPictureBack = card.getPathToPictureBack();
        if (fieldFlags.contains(EditTexts.CARD_BACK_IMAGE) && pathToPictureBack.equals("") == false) {
            ImageView backImage = (ImageView) solo.getView(R.id.image_card_details_back);
            Bitmap backPic = picHelper.GetScaledImageFromPath(card.getPathToPictureFront(), 120, 120);
